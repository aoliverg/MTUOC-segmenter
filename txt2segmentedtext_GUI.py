...
def select_input_file():
    inputfile = askopenfilename(initialdir = ".",filetypes =(("text files", ["*.txt"]),("All Files","*.*")),
                           title = "Choose a file to segment.")
    E1.delete(0,END)
...
top = Tk()
top.title("txt2segmentedtext_GUI")

B1=tkinter.Button(top, text = str("Input file"), borderwidth = 1, command=select_input_file,width=14).grid(row=0,column=0)
